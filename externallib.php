...
class local_webservice_external extends external_api {
    ...
    public static function webservice_surveycheck($courseid = 0, $feedbackid = 0) {
        ...
        switch(true)
        {
            ...
            case($courseid > 0 && $feedbackid >0):
                $return=array();
                $textresponses = $DB->get_records_sql('SELECT qrt.id as id, cc.name as category, c.fullname as coursename, q.name as questionnaire, qq.type_id, qq.length, q.intro as info, qq.name as sectioncategory, qq.content as question, qrt.response as response FROM {questionnaire} AS q 
                ...
                $rankresponses = $DB->get_records_sql('SELECT qrr.id as id, cc.name as category, c.fullname as coursename, q.name as questionnaire, qq.type_id, qq.length, q.intro as info, qq.name as sectioncategory, qqc.content as question, qrr.rank as response FROM {questionnaire} AS q
                                                        INNER JOIN {course} AS c ON (c.id = q.course AND c.id = ? AND q.id = ?)
                                                        INNER JOIN {course_categories} AS cc ON (cc.id = c.category)
                                                        INNER JOIN {questionnaire_question} AS qq ON (qq.survey_id = q.id)
                                                        ...
                                                        WHERE q.intro like "<ul>%" AND cc.id != 39', array($courseid,$feedbackid));
                $dateresponses = $DB->get_records_sql('SELECT qrd.id as id, cc.name as category, c.fullname as coursename, q.name as questionnaire, qq.type_id, qq.length, q.intro as info, qq.name as sectioncategory, qq.content as question, qrd.response as response FROM {questionnaire} AS q
                                                        INNER JOIN {course} AS c ON (c.id = q.course AND c.id = ? AND q.id = ?)
                                                        INNER JOIN {course_categories} AS cc ON (cc.id = c.category)
                                                        INNER JOIN {questionnaire_question} AS qq ON (qq.survey_id = q.id)
                                                        INNER JOIN {questionnaire_response_date} AS qrd ON (qrd.question_id = qq.id)
                                                        WHERE q.intro like "<ul>%" AND cc.id != 39', array($courseid,$feedbackid));
                $boolresponses = $DB->get_records_sql('SELECT qrd.id as id, cc.name as category, c.fullname as coursename, q.name as questionnaire, qq.type_id, qq.length, q.intro as info, qq.name as sectioncategory, qq.content as question, qrd.choice_id as response FROM {questionnaire} AS q
                                                        INNER JOIN {course} AS c ON (c.id = q.course AND c.id = ? AND q.id = ?)
                                                        INNER JOIN {course_categories} AS cc ON (cc.id = c.category)
                                                        INNER JOIN {questionnaire_question} AS qq ON (qq.survey_id = q.id)
                                                        INNER JOIN {questionnaire_response_bool} AS qrd ON (qrd.question_id = qq.id)
                                                        WHERE q.intro like "<ul>%" AND cc.id != 39', array($courseid,$feedbackid));
                $singleresponses = $DB->get_records_sql('SELECT qrs.id as id, cc.name as category, c.fullname as coursename, q.name as questionnaire, qq.type_id, qq.length, q.intro as info, qq.name as sectioncategory, qq.content as question, qqc.content as response FROM {questionnaire} AS q
                                                        INNER JOIN {course} AS c ON (c.id = q.course AND c.id = ? AND q.id = ?)
                                                        INNER JOIN {course_categories} AS cc ON (cc.id = c.category)
                                                        INNER JOIN {questionnaire_question} AS qq ON (qq.survey_id = q.id)
                                                        INNER JOIN {questionnaire_quest_choice} AS qqc ON (qqc.question_id = qq.id)
                                                        ...
                                                        WHERE q.intro like "<ul>%" AND cc.id != 39', array($courseid,$feedbackid));
                $multiresponses = $DB->get_records_sql('SELECT qrm.id as id, cc.name as category, c.fullname as coursename, q.name as questionnaire, qq.type_id, qq.length, q.intro as info, qq.name as sectioncategory, qq.content as question, qqc.content as response FROM {questionnaire} AS q
                                                        INNER JOIN {course} AS c ON (c.id = q.course AND c.id = ? AND q.id = ?)
                                                        INNER JOIN {course_categories} AS cc ON (cc.id = c.category)
                                                        INNER JOIN {questionnaire_question} AS qq ON (qq.survey_id = q.id)
                                                        INNER JOIN {questionnaire_quest_choice} AS qqc ON (qqc.question_id = qq.id)
